...
export async function POST(req: Request) {
    ...
    if (contentType.includes("multipart/form-data")) {
        ...
        // 1️⃣ Extract text using GPT-4o Vision
        const visionRes = await openai.chat.completions.create({
            model: "gpt-4o-mini",
            messages: [
                {
                    role: "system",
                    content: "Extract ONLY the ingredient list from the food label. Return plain text only."
                },
                {
                    role: "user",
                    content: [
    ...
    // ======================================================
    // 💬 CASE 2: NORMAL CHAT (your existing logic)
    // ======================================================
                ...
                const stream = createUIMessageStream({
                    execute({ writer }) {
                        ...
                        writer.write({
                            type: 'text-delta',
                            id: textId,
                            delta: moderationResult.denialMessage 
                                || "Your message violates guidelines.",
                        });
